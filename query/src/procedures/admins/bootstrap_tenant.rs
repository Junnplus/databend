...
impl BootstrapTenantProcedure {
    ...
    pub fn desc() -> ProcedureDescription {
        ProcedureDescription::creator(Box::new(Self::try_create))
            .features(ProcedureFeatures::default().num_arguments(5))
    }
}
...
impl Procedure for BootstrapTenantProcedure {
    async fn eval(&self, ctx: Arc<QueryContext>, args: Vec<String>) -> Result<DataBlock> {
        if !ctx.get_config().query.management_mode {
            return Err(ErrorCode::ManagementModePermissionDenied(
                "Access denied: 'BOOTSTRAP_TENANT' only used in management-mode",
            ));
        }
